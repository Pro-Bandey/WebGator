(() => {
  ...
  function attachDragEvents() {
    ...
    document.addEventListener("mouseup", async () => {
      if (!isDragging) return;
      isDragging = false;
      nav.style.transition = "transform 0.3s cubic-bezier(0.22, 1, 0.36, 1), opacity 0.3s ease";
  ...
  function attachNavEvents() {
    nav.addEventListener("click", (e) => {
      ...
      const actions = {
        ...
        close: () => {
          window.close();
          setTimeout(() => alert("Chrome security prevents scripts from closing unowned tabs."), 200);
        },
      };
  ...
  function getDominantBackgroundColor() {
    ...
    if (bg === 'rgba(0, 0, 0, 0)' || bg === 'transparent') {
      ...
      bg = isDark ? 'rgb(32, 33, 36)' : 'rgb(255, 255, 255)';
    }
  ...
  function applyNativeTheme() {
    ...
    nav.style.setProperty('--wg-icon-color', isLight ? '#202124' : '#F8F9FA'); // Chrome specific grey/white
    nav.style.setProperty('--wg-hover-bg', isLight ? 'rgba(0, 0, 0, 0.06)' : 'rgba(255, 255, 255, 0.1)');
    nav.style.setProperty('--wg-active-bg', isLight ? 'rgba(0, 0, 0, 0.12)' : 'rgba(255, 255, 255, 0.15)');

    nav.style.setProperty('--wg-tooltip-bg', isLight ? '#202124' : '#E8EAED');
    nav.style.setProperty('--wg-tooltip-text', isLight ? '#FFFFFF' : '#202124');
  }
